...
def plot_triplet(img_hw: np.ndarray, pred_hw: np.ndarray, gt_hw: np.ndarray, case_id: str,
    ...
    CLASS_COLORS = [
        (0.1, 0.1, 0.1),       # 0 - background (black)
        (0.8, 0.4, 0.4),       # 1 - soft red
        (0.4, 0.8, 0.4),       # 2 - soft green
        (0.4, 0.4, 0.8),       # 3 - soft blue
        (0.9, 0.8, 0.4),       # 4 - light yellow
        (0.8, 0.6, 0.8),       # 5 - lavender
    ]
    ...
    # 2) Prediction
    plt.subplot(1, 3, 2)
    plt.imshow(pred_hw, cmap=cmap, norm = norm)
    plt.title("Prediction")
    plt.axis("off")

    # 3) Ground truth
    plt.subplot(1, 3, 3)
    plt.imshow(gt_hw, cmap=cmap, norm = norm)
